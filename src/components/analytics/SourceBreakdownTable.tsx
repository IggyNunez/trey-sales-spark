...
interface SourceBreakdownTableProps {
  ...
  onRowClick?: (source: string, dateType: 'completed' | 'created') => void;
}
  ...
  if (isLoading) {
    return (
      ...
      <CardHeader className="pb-3">
        <CardTitle className="text-sm font-medium">Lead Source ({title})</CardTitle>
        </CardHeader>
        <CardContent>
...
export function SourceBreakdownTable({ startDate, endDate, bookingPlatform, onRowClick }: SourceBreakdownTableProps) {
  const { sourceCompleted, sourceCreated, isLoading } = usePlatformAggregates({
    startDate,
    endDate,
    bookingPlatform,
  });
